        ...
        for index in range(len(baseData)):
            ...
            row.append(
                processData(dataframe=dataframe,
                            ...
                            failureProb=baseData[index] if baseIndex == 2 else baseDefault[2],
...
def dataProcessAndPlotPerSetting(filename, targets, base, baseIndex, baseDefault, baseAbbr):
    ...
    for period in periods:
        dataForPlot = collectData(dataframe=dataframe,
                                  policies=policies,
                                  targets=targets,
                                  baseData=baseData,
                                  baseIndex=baseIndex,
                                  ...
                                  period=period)
        # plot figures
        plotName = targets[0]+"_for_" + baseAbbr + "_and_"+period+".pdf"
        plotData(datatable=dataForPlot,
                 xaxis=baseData,
                 labels=policies,
                 name=plotName)
